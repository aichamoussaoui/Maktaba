...
class BookRepository {
    ...
    private val booksList = listOf(
        ...
        Book(isbn = "978-0-13-764554-1", title = "Android Programming: The Big Nerd Ranch Guide", nbPages = 688)
    )
    ...
    fun searchBooks(query: String): List<Book> {
        val q = query.trim()
        if (q.isEmpty()) return booksList

        return booksList.filter { book ->
            book.title.contains(q, ignoreCase = true) ||
            book.isbn.contains(q, ignoreCase = true)
        }
    }
}
